    ...
    <div className="min-h-screen bg-zinc-950 text-white flex font-sans">
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="bg-zinc-900 border-b border-zinc-800 px-8 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-black text-white">Tableau de bord</h1>
            <p className="text-xs text-zinc-500">Bienvenue, voici l'état du site aujourd'hui.</p>
          </div>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
            {stats.map((s) => (
              <div key={s.label} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                ...
                <p className="text-xl font-black text-white mb-2">{s.value}</p>
              ...
              <div className="flex items-end gap-2 h-40">
                {monthlySales.map((val, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    />
                    <span className="text-xs text-zinc-600">{months[i]}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <thead>
                <tr className="border-b border-zinc-800">
                  {["Réf.", "Client", "Date", "Total", "Statut", "Action"].map((h) => (
                    <th key={h} className="text-left px-6 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {recentOrders.map((order) => (
                  <tr key={order.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30 transition-colors">
                    <td className="px-6 py-4 text-xs font-mono text-zinc-400">{order.id}</td>
                    <td className="px-6 py-4 text-sm text-white font-medium">{order.client}</td>
                    <td className="px-6 py-4 text-xs text-zinc-500">{order.date}</td>
                    <td className="px-6 py-4 text-sm font-bold text-orange-500">{formatPrice(order.total)}</td>
